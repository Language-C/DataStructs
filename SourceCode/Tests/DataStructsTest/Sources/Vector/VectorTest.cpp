...
namespace VectorTest
{
    ...
    public:
        ...
        TEST_METHOD(CreateWithCapacity_5) {
            Vector* vector = VectorCreateWithCapacity(5U);

            Assert::AreEqual(VectorCapacity(vector), 5U);
            VectorDestroyWithFreeElements(&vector);
        }

        TEST_METHOD(CreateWithCapacity_0) {
            Vector* vector = VectorCreateWithCapacity(0U);
    ...
    public:
        ...
        TEST_METHOD(ReturnTrue) {
            Assert::AreEqual(VectorIsEmpty(vector), True);
        }

        TEST_METHOD(ReturnFalse) {
            VectorAdd(vector, new int(5));
            Assert::AreEqual(VectorIsEmpty(vector), False);
        }
    };
